    ...
    with urllib.request.urlopen(f"https://www.ncbi.nlm.nih.gov/pmc/?term={id}") as url2:
      for line in url2:
        ...
        if line.find(f'/pmc/articles/PMC{id}/pdf/') >= 0:
          urlitem = line[line.find(f"/pmc/articles/PMC{id}/pdf/"):line.find(".pdf", line.find(f"/pmc/articles/PMC{id}/pdf/"))+4].strip()
...
def get_pub_ids(query):
    ...
    res = requests.get(url = f'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pmc&term={esc}&retmax=10000&sort=relevance')
