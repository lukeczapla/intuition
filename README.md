## Literature curation and other services, OncoKB curation assistant, Q&A, and NGS APIs
 ...
 * GCA_009914755.4_T2T-CHM13v2.0_genomic.fna from NCBI is used as the data input, and Test1.readGenes() works with the NCBI data for gene sequences and also RNA transcripts, validating their locations and adding to the Targets collection in MongoDB.

improvements: The one greatest biggest criteria is on the PDF2HTML (and the other paragraph / structured table + figures endpoint delivered as JSON) for the advanced cases, it has to be good enough to ace the edge cases (latest graphs in some PMIDs >~ 33000000 encoded with newer approaches - great for PDF highlighting but bad for sectionalizing the photo/image components) to fully upgrade to the newest pipeline.  Some work has been added to provide better textArticle (JSON) and HTMLArticle controllers that improve the rendering of the articles and reprocessing should be done, including for the Q&A with vector scoring.
