...
class Suggestions extends React.Component {
  constructor(props) {
    ...
    autoBind(this, 'handleMouseLeave', 'handleMouseMove', 'renderSuggestion', 'setFocusedSuggestion', 'scrollToSuggestion');
  }
  ...
  renderSuggestion(suggestion, index) {
    ...
    return (
      <Suggestion className={classNames({[props.styles.suggestion]: true, [props.styles.suggestionFocused]: isFocused})} index={index} key={suggestion} onClick={props.onSelection} onMouseMove={this.handleMouseMove} ref={isFocused && this.setFocusedSuggestion} searchTerm={props.searchTerm} suggestion={suggestion} suggestionRenderer={props.suggestionRenderer}/>
    );
  }

  render() {
    return (
      <ul style={{zIndex: 2}} className={this.props.styles.suggestions} ref={ref => (this.list = ref)} onMouseLeave={this.handleMouseLeave}>
