...
class SearchBar extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.count !== this.props.count) this.clearInput();
  }
  ...
  renderSuggestions(searchTerm, styles) {
    ...
    if (searchTerm.length > 2) return (
      <Suggestions focusedSuggestion={this.state.focusedSuggestion} onSelection={this.handleSelection} onSuggestionHover={this.handleHover} searchTerm={searchTerm} styles={styles} suggestions={this.props.suggestions} suggestionRenderer={this.props.suggestionRenderer}/>
    );
  ...
  render() {
    ...
    return (
      <div className={styles.wrapper} ref={ref => (this.container = ref)}>
        <div className={classNames({[styles.field]: true, [styles.fieldFocused]: state.isFocused, [styles.hasSuggestions]: props.suggestions.length > 0})}>
          <b style={{backgroundColor: "white", lineHeight: "3em", paddingLeft: "1em"}}> Find</b><input {...this.attributes} className={styles.input} type="text" ref={ref => (this.input = ref)} value={state.value} onChange={this.handleChange} onFocus={this.toggleFocus} onBlur={this.toggleFocus} onKeyDown={props.suggestions && this.handleKeyDown}/>
